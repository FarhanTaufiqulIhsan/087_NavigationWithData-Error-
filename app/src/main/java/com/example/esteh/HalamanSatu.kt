...
{
    var rasaYangDipilih by rememberSaveable { mutableStateOf("")}
        ...
        Column(modifier = Modifier.padding(dimensionResource(id = R.dimen.padding_medium))){
            pilihanRasa.forEach { item ->
                Row (modifier = Modifier.selectable(
                    selected = rasaYangDipilih == item,
                    onClick = {
                        rasaYangDipilih = item
                        onSelectionChanged(item)
                    }
                ),
                    verticalAlignment = Alignment.CenterVertically
                    ){
                    RadioButton(selected = rasaYangDipilih == item,
                        onClick = {
                            rasaYangDipilih = item
                            onSelectionChanged(item)
                        }
            ...
            Row (modifier = Modifier
                ...
                Button(modifier = Modifier.weight(1f),
                    ...
                    onClick = { onConfirmButtonClick(textJmlBeli.toInt()) }
                ) {
